...
if __name__ == '__main__':
    ...
    lista_instancias_abiertas = df_root[(df_root['QueryState'] == 'OPEN') & (df_root['TypeQuery'] == 'QUERY')]['FormFieldInstance Id'].to_list()
    ...
    date_of_visit = date_of_visit(df_root, path_excel_writer, lista_instancias_abiertas)
    informed_consent_revision= informed_consent_revision(df_root, path_excel_writer, lista_instancias_abiertas)
    demographic = demographic(df_root, path_excel_writer, lista_instancias_abiertas)
    child_bearing_potential = child_bearing_potential(df_root, path_excel_writer, lista_instancias_abiertas)
    history_of_cutaneous_leishmaniasis = history_of_cutaneous_leishmaniasis(df_root, path_excel_writer, lista_instancias_abiertas)
    eligibility = eligibility(df_root, path_excel_writer, lista_instancias_abiertas)
    Medical_or_surgical_history = Medical_or_surgical_history(df_root, path_excel_writer, lista_instancias_abiertas)
    vein_assesment = vein_assesment(df_root, path_excel_writer, lista_instancias_abiertas)
    urinary_drug_screen = urinary_drug_screen(df_root, path_excel_writer, lista_instancias_abiertas)
    pregnancy_test = pregnancy_test(df_root, path_excel_writer, lista_instancias_abiertas)
    clinical_laboratory_test_hematology = clinical_laboratory_test_hematology(df_root, path_excel_writer, lista_instancias_abiertas)
    clinical_laboratory_test_clinical_chemistry = clinical_laboratory_test_clinical_chemistry(df_root, path_excel_writer, lista_instancias_abiertas)
    clinical_laboratory_test_coagulation = clinical_laboratory_test_coagulation(df_root, path_excel_writer, lista_instancias_abiertas)
    immunoassay = immunoassay(df_root, path_excel_writer, lista_instancias_abiertas)
    urinalysis = urinalysis(df_root, path_excel_writer, lista_instancias_abiertas)
    urine_microscopic_examination = urine_microscopic_examination(df_root, path_excel_writer, lista_instancias_abiertas)
    virology = virology(df_root, path_excel_writer, lista_instancias_abiertas)
    lesion_measurement = lesion_measurement(df_root, path_excel_writer, lista_instancias_abiertas)
    physical_examination = physical_examination(df_root, path_excel_writer, lista_instancias_abiertas)
    vital_signs = vital_signs(df_root, path_excel_writer, lista_instancias_abiertas)
    lead_ECG = lead_ECG(df_root, path_excel_writer, lista_instancias_abiertas)

    # --------------------- D-1 --------------------------------------
    covid_19_testing = covid_19_testing(df_root, path_excel_writer, lista_instancias_abiertas)
    alcohol_screen = alcohol_screen(df_root, path_excel_writer, lista_instancias_abiertas)
    clinical_laboratory_test_clinical_chemistry_D_1 = clinical_laboratory_test_clinical_chemistry_D_1(df_root, path_excel_writer, lista_instancias_abiertas)
    interleukin_6 = interleukin_6(df_root, path_excel_writer, lista_instancias_abiertas)
    titration_of_auto_antibodies = titration_of_auto_antibodies(df_root, path_excel_writer, lista_instancias_abiertas)

    # --------------------- D1 --------------------------------------
    injection_site_examination = injection_site_examination(df_root, path_excel_writer, lista_instancias_abiertas)
    PBMC_isolate = PBMC_isolate(df_root, path_excel_writer, lista_instancias_abiertas)
    Pharmacokinetic_blood_sampling = Pharmacokinetic_blood_sampling(df_root, path_excel_writer, lista_instancias_abiertas)
    pharmacodynamic_blood_sampling = pharmacodynamic_blood_sampling(df_root, path_excel_writer, lista_instancias_abiertas)
    mRNA_markers = mRNA_markers(df_root, path_excel_writer, lista_instancias_abiertas)

    # --------------------- unscheduled --------------------------------------
    adminsitration_CpG_ODN = adminsitration_CpG_ODN(df_root, path_excel_writer, lista_instancias_abiertas)
    miltefosine_administration = miltefosine_administration(df_root, path_excel_writer, lista_instancias_abiertas)
    prior_concomitant_medication = prior_concomitant_medication(df_root, path_excel_writer, lista_instancias_abiertas)
    prior_concomitant_procedures = prior_concomitant_procedures(df_root, path_excel_writer, lista_instancias_abiertas)
    ev_med_proce_treatment = ev_med_proce_treatment(df_root, path_excel_writer, lista_instancias_abiertas)
    end_of_study = end_of_study(df_root, path_excel_writer, lista_instancias_abiertas)
    adverse_events = adverse_events(df_root, path_excel_writer, lista_instancias_abiertas)
